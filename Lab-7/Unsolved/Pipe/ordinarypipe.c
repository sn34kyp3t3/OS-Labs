...
int main(void)
{
	...
	pid_t ret;
	char buffer[BUFFERSIZE]; //this buffer is where the data is to be kept
	if(pipe(pipefd)==-1)
	{
	...
	if(ret>0) //parent process
	{
		...
		printf("\n I am Parent. Sending money to my child.\n");
		write(pipefd[1], "money from Parent.",BUFFERSIZE); //this is to write the contents into write end of the pipe i.e pipefd[1]. BUFFERSIZE means we are writing __ characters to the buffer including ‘\0’
		close(pipefd[1]);
	}
//understand like parent has deposited some money and then child takes it.
	if(ret==0)
	{
		...
		sleep(1); //child is made to sleep till parent writes the contents.
		fflush(stdin);
		printf("\n I am CHILD. Received\n");
		sleep(1);
		//data already written at the write end can be read from here.
		read(pipefd[0], buffer, sizeof(buffer));
		// the data is now read, but we need to display on the screen. For that purpose we are keeping the data in buffer. From buffer we can write it to the display.
